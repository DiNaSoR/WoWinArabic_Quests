...
function QTR_ExpandUnitInfo(msg, OnObjectives)
   ...
   msg = string.gsub(msg, "$N$", AS_UTF8reverse(string.upper(QTR_name)));
...
function GS_ON_OFF()
   if (QTR_curr_goss == "1") then -- wyłącz tłumaczenie - pokaż oryginalny tekst
      ...
      QTR_curr_goss = "1";
      GossipGreetingText:SetText(QTR_GS2[QTR_curr_hash]);
      GossipGreetingText:SetFont(QTR_Font2, tonumber(QTR_PS["fontsize"]));
      QTR_ToggleButtonGS1:SetText("Gossip-Hash=" .. tostring(QTR_curr_hash) .. " AR");
...
function GS_ON_OFF2()
   if (QTR_curr_goss == "1") then -- wyłącz tłumaczenie - pokaż oryginalny tekst
      ...
      GreetingText:SetText(QTR_ExpandUnitInfo(Greeting_TR, false));
...
function QTR_Gossip_Show()
   ...
   if (Nazwa_NPC) then
      ...
      if (Greeting_Text and (string.find(Greeting_Text, " ") == nil)) then -- nie jest to tekst po arabsku (nie ma twardej spacji)
         Nazwa_NPC = string.gsub(Nazwa_NPC, '"', '\"');
         local Origin_Text = QTR_DetectAndReplacePlayerName(Greeting_Text);
         local Czysty_Text = QTR_DeleteSpecialCodes(Origin_Text);
         ...
         if (GS_Gossip[Hash]) then                                      -- istnieje tłumaczenie tekstu GOSSIP tego NPC
            local Greeting_TR = QTR_ExpandUnitInfo(GS_Gossip[Hash], false);
            if (string.sub(Nazwa_NPC, 1, 17) == "Bronze Timekeeper") then -- wyścigi na smokach - wyjątej z sekundami: $1.$2 oraz $3.$4
               local wartab = { 0, 0, 0, 0, 0, 0 };                     -- max. 6 liczb całkowitych w tekście
               local arg0 = 0;
               for w in string.gmatch(strtrim(Greeting_Text), "%d+") do
                  arg0 = arg0 + 1;
                  if (math.floor(w) > 999999) then
                     wartab[arg0] = tostring(math.floor(w)):reverse():gsub("(%d%d%d)(%d%d%d)", "%1.%2."):gsub("(%-?)$",
                        "%1"):reverse();                                                                                                 -- tu mamy kolejne cyfry z oryginału
                  elseif (math.floor(w) > 99999) then
                     wartab[arg0] = tostring(math.floor(w)):reverse():gsub("(%d%d%d)(%d%d%d)", "%1.%2"):gsub("(%-?)$",
                        "%1"):reverse();                                                                                                 -- tu mamy kolejne cyfry z oryginału
                  elseif (math.floor(w) > 999) then
                     wartab[arg0] = tostring(math.floor(w)):reverse():gsub("(%d%d%d)", "%1."):gsub("(%-?)$", "%1")
                     :reverse();                                                                                                         -- tu mamy kolejne cyfry z oryginału
                  else
                     wartab[arg0] = w;                                                                                                   -- tu mamy kolejne liczby całkowite z oryginału
                  end
            ...
            if (GossipTextFrame) then
               QTR_ToggleButtonGS1:SetText("Gossip-Hash=" .. tostring(Hash) .. " AR");
               QTR_ToggleButtonGS1:Enable();
               GossipGreetingText = GossipTextFrame.GreetingText;
               local GO_height = GossipGreetingText:GetHeight();
               if (QTR_first_gs_show == 0) then
                  QTR_first_gs_show = 1;
                  local Text_tmp = AS_ReverseAndPrepareLineText(Greeting_TR, GossipGreetingText:GetWidth(),
                     tonumber(QTR_PS["fontsize"]));
               end
               Greeting_TR = AS_ReverseAndPrepareLineText(Greeting_TR, GossipGreetingText:GetWidth(),
                  tonumber(QTR_PS["fontsize"]));
               QTR_GS2[Hash] = Greeting_TR;
               GossipGreetingText:SetText(Greeting_TR);
               GossipGreetingText:SetFont(QTR_Font2, tonumber(QTR_PS["fontsize"]));
            ...
            if (QTR_PS["saveGS"] == "1") then
               ...
               if (C_Map.GetBestMapForUnit("player")) then
                  QTR_GOSSIP[Nazwa_NPC .. "@" .. tostring(Hash) .. "@" .. C_Map.GetBestMapForUnit("player")] =
                  Origin_Text .. "@" .. QTR_name .. ":" .. QTR_race .. ":" .. QTR_class;
               else
                  QTR_GOSSIP[Nazwa_NPC .. "@" .. tostring(Hash) .. "@0"] = Origin_Text ..
                  "@" .. QTR_name .. ":" .. QTR_race .. ":" .. QTR_class;
               end
            end
         end
      end
      ...
      for _, GTxtframe in GossipFrame.GreetingPanel.ScrollBox:EnumerateFrames() do -- pobierz obiekty enumeryczne
         ...
         if (GTxtframe.GreetingText) then                                         -- Greeting Text
            ...
            if (((GTtype == 3) or (GTtype == 4) or (GTtype == 5)) and (QTR_PS["gossipON"] == "1") and (string.find(GTxtframe:GetText(), " ") == nil)) then -- gossip options
               local GOptionText = QTR_DetectAndReplacePlayerName(GTxtframe:GetText());
               local prefix = "";
               local sufix = "";
               ...
               if (GS_Gossip[OptHash]) then -- jest tłumaczenie
                  ...
                  if (GTxtframe.Icon) then
                     IconLen = GTxtframe.Icon:GetWidth() + 24;
                     a1, a2, a3, a4, a5 = GTxtframe.Icon:GetPoint();
                     if (a4 < 10) then
                        GTxtframe.Icon:SetPoint(a1, a2, a3, a4 + GTxtframe:GetWidth() - IconLen, a5);
                     end
                     IconLen = IconLen + 16;
                  end
                  local Greeting_TR = QTR_ExpandUnitInfo(GS_Gossip[OptHash], false);
                  if (QTR_first_gs_show == 0) then
                     QTR_first_gs_show = 1;
                     local Text_tmp = AS_ReverseAndPrepareLineText(Greeting_TR, GTxtframe:GetWidth() - IconLen,
                        tonumber(QTR_PS["fontsize"]));
                  end
                  Greeting_TR = AS_ReverseAndPrepareLineText(Greeting_TR, GTxtframe:GetWidth() - IconLen,
                     tonumber(QTR_PS["fontsize"]));
                  local transTR = prefix .. Greeting_TR .. sufix .. " "; -- twarda spacja na końcu
                  QTR_goss_optionsEN[GTxtframe] = GOptionText;           -- zapis tekstu oryginalnego gossip option
                  QTR_goss_optionsTR[GTxtframe] = transTR;               -- zapis tekstu tureckiego gossip option
                  GTxtframe:SetText(transTR);
               else
                  if (QTR_PS["saveGS"] == "1") then
                     -- zapis do pliku
                     if (C_Map.GetBestMapForUnit("player")) then
                        QTR_GOSSIP[Nazwa_NPC .. "@" .. tostring(OptHash) .. "@" .. C_Map.GetBestMapForUnit("player")] =
                        GOptionText .. "@" .. QTR_name .. ":" .. QTR_race .. ":" .. QTR_class;
                     else
                        QTR_GOSSIP[Nazwa_NPC .. "@" .. tostring(OptHash) .. "@0"] = GOptionText ..
                        "@" .. QTR_name .. ":" .. QTR_race .. ":" .. QTR_class;
                     end
                  end
               end
               ...
               for k, v in pairs(regions) do
                  if (v:GetObjectType() == "FontString") then
                     v:SetFont(QTR_Font2, tonumber(QTR_PS["fontsize"]));
                     --                     QTR_goss_optionsFONT[GTxtframe] = v;     -- zapis obiektu z czcionką tekstu arabskiego
                  end
               end
               GTxtframe:Resize();
...
function GossipOnQuestFrame()                                         -- frame: QuestFrame
   if ((GreetingText:IsVisible()) and (QTR_PS["gossipON"] == "1")) then -- mamy gossip w QuestFrame i włączone wyświetlanie tłumaczeń gossip
      QTR_ToggleButton0:Disable();                                    -- wyłącz możliwość przełączania EN-AR
      ...
      if (Greeting_Text and (string.find(Greeting_Text, " ") == nil)) then -- nie jest to tekst po turecku (nie ma twardej spacji)
         local GO_resized = 0;
         QTR_goss_optionsEN = {};                                       -- wyzeruj tablicę na opcje EN gossip
         QTR_goss_optionsTR = {};                                       -- wyzeruj tablicę na opcje AR gossip
         local Origin_Text = QTR_DetectAndReplacePlayerName(Greeting_Text);
         local Czysty_Text = QTR_DeleteSpecialCodes(Origin_Text);
         local Hash = StringHash(Czysty_Text);
         QTR_curr_hash = Hash;
         ...
         if (GS_Gossip[Hash]) then     -- istnieje tłumaczenie tekstu GOSSIP dla tego hasha
            QTR_ToggleButton0:SetText("Gossip-Hash=" .. tostring(Hash) .. " AR");
            ...
            if (QTR_first_gs_show == 0) then
               QTR_first_gs_show = 1;
               local Text_tmp = AS_ReverseAndPrepareLineText(Greeting_TR, GreetingText:GetWidth(),
                  tonumber(QTR_PS["fontsize"]));
            end
            Greeting_TR = AS_ReverseAndPrepareLineText(Greeting_TR, GreetingText:GetWidth(), tonumber(QTR_PS["fontsize"]));
            QTR_GS2[Hash] = Greeting_TR;
            GreetingText:SetText(Greeting_TR);
            GreetingText:SetFont(QTR_Font2, tonumber(QTR_PS["fontsize"]));
            --            GreetingText:Resize();
            QTR_curr_goss = "1";
         ...
         if (QTR_PS["gossipON"] == "1") then
            for GText in QuestFrameGreetingPanel.titleButtonPool:EnumerateActive() do -- options in gossip QuestFrame
               ...
               local GOptionText = QTR_DetectAndReplacePlayerName(GossText);
               ...
               if (GS_Gossip[TitleHash]) then
                  ...
                  local Greeting_TR = QTR_ExpandUnitInfo(GS_Gossip[TitleHash], false);
                  if (QTR_first_gs_show == 0) then
                     QTR_first_gs_show = 1;
                     local Text_tmp = AS_ReverseAndPrepareLineText(Greeting_TR, GText:GetWidth() - IconLen,
                        tonumber(QTR_PS["fontsize"]));
                  end
                  Greeting_TR = AS_ReverseAndPrepareLineText(Greeting_TR, GText:GetWidth() - IconLen,
                     tonumber(QTR_PS["fontsize"]));
                  local transTR = prefix .. Greeting_TR .. sufix; -- twarda spacja na końcu
                  QTR_goss_optionsEN[GText] = GText:GetText();    -- zapis tekstu oryginalnego gossip option
                  QTR_goss_optionsTR[GText] = transTR;            -- zapis tekstu arabskiego gossip option
                  GText:SetText(transTR);
               else
                  if (QTR_PS["saveGS"] == "1") then
                     local Nazwa_NPC = QuestFrameTitleText:GetText();
                     GossText = QTR_DetectAndReplacePlayerName(GossText);
                     GossText = string.gsub(GossText, '"', '\"');
                     if (C_Map.GetBestMapForUnit("player")) then
                        QTR_GOSSIP[Nazwa_NPC .. '@' .. tostring(TitleHash) .. "@" .. C_Map.GetBestMapForUnit("player")] =
                        GOptionText .. "@" .. QTR_name .. ":" .. QTR_race .. ":" .. QTR_class;
                     else
                        QTR_GOSSIP[Nazwa_NPC .. '@' .. tostring(TitleHash) .. "@0"] = GOptionText ..
                        "@" .. QTR_name .. ":" .. QTR_race .. ":" .. QTR_class;
                     end
                  end
               end
...
function QTR_ReplaceOnlyWholeWords(txt, finder, replacer)
   local result = txt;
   local last = 1;
   ...
   while (nr_poz and nr_poz > 0) do                    -- OK, znalazł coś, indeksuje pozycję początkową od 1
      if ((nr_poz == 1) or ((nr_poz > 1) and (string.sub(result, nr_poz - 1, nr_poz - 1) == ' ')) or ((nr_poz > 2) and (string.sub(result, nr_poz - 2, nr_poz - 1) == '$B'))) then
         ...
         if ((char_after == '.') or (char_after == ',') or (char_after == '?') or (char_after == '!') or (char_after == ' ') or (char_after == ';') or (char_after == ':') or (char_after == '>')) then
...
function QTR_DetectAndReplacePlayerName(txt, target)
   if (txt == nil) then return ""; end
   local text = string.gsub(txt, '\r', "");
   text = string.gsub(text, '\n', "$B");
   text = QTR_ReplaceOnlyWholeWords(text, QTR_name, "$N");
   text = QTR_ReplaceOnlyWholeWords(text, string.upper(QTR_name), '$N$');
   text = QTR_ReplaceOnlyWholeWords(text, QTR_race, '$R');
   text = QTR_ReplaceOnlyWholeWords(text, string.lower(QTR_race), '$R');
   text = QTR_ReplaceOnlyWholeWords(text, string.upper(QTR_race), '$R$');
   text = QTR_ReplaceOnlyWholeWords(text, QTR_class, '$C');
   text = QTR_ReplaceOnlyWholeWords(text, string.lower(QTR_class), '$C');
   text = QTR_ReplaceOnlyWholeWords(text, string.upper(QTR_class), '$C$');
   if (target) then
      text = QTR_ReplaceOnlyWholeWords(text, target, "$N");
   end
   return text;
end
...
function QTR_DeleteSpecialCodes(txt)
   if (txt == nil) then return ""; end
   ...
   text = string.gsub(text, '$C', '');
   text = string.gsub(text, 'YOUR_NAME', '');
   text = string.gsub(text, 'YOUR_RACE', '');
   text = string.gsub(text, 'YOUR_CLASS', '');
...
QTR_f:RegisterEvent("GOSSIP_SHOW");
